...
void BaseSocket::SetProtocolVersion_IPv4() { hints.ai_family = AF_INET; }

void BaseSocket::SetProtocolVersion_IPv6() { hints.ai_family = AF_INET6; }
...
int BaseSocket::Recv(char* _recvbuf, int recvbuf_size, const int flg) {
   ...
   } catch (const std::exception& e) { std::cerr << "Exception Error at Recv():" << e.what() << std::endl; }
...
int BaseSocket::Recvfrom(B_ADDRESS_IN* _senderAddr, char* _recvbuf, int recvbuf_size, const int flg) {
   ...
   try {
      ...
      byteSize = recvfrom(m_socket, _recvbuf, recvbuf_size, 0, (struct sockaddr*)_senderAddr, &sendAddrSize);
#endif
   } catch (const std::exception& e) { std::cerr << "Exception Error at Recvfrom():" << e.what() << std::endl; }
...
int BaseSocket::Send(const char* _sendData, const int _sendDataSize) {
   ...
   } catch (const std::exception& e) { std::cerr << "Exception Error at Send():" << e.what() << std::endl; }
   return len;
}
...
int BaseSocket::Sendto(const char* _sendData, const int _sendDataSize) {
   ...
   } catch (const std::exception& e) { std::cerr << "Exception Error at Sendto():" << e.what() << std::endl; }
