...
void TCP_Routine::Update(const std::shared_ptr<BaseSocket> _socket, std::vector<char> &_recvData, std::queue<std::vector<char>> &_recvDataQueList)
{
	...
	if (dataSize > 0)
	{
		...
		int nowSize = _recvData.size();
		_recvData.resize(nowSize + dataSize);
		std::memcpy((char *)&_recvData[nowSize], &buf[0], dataSize);
		while (_recvData.size() > sizeof(int))
		{
			int dataSize;
			std::memcpy(&dataSize, &_recvData[0], sizeof(int));
			...
			if (_recvData.size() > dataSize)
			{
				std::vector<char> addData;
				addData.resize(dataSize);
				std::memcpy(&addData[0], &_recvData[sizeof(int)], dataSize);
				_recvDataQueList.push(addData);
				_recvData.erase(_recvData.begin(), _recvData.begin() + dataSize + sizeof(int));
			}
		}
...
void TCP_Routine::Update(std::vector<std::shared_ptr<BaseSocket>> &_clientList, std::unordered_map<int, std::vector<char>> &_recvDataMap, std::queue<std::pair<int, std::vector<char>>> &_recvDataQueList)
{
	...
	for (int i = 0; i < _clientList.size(); i++)
	{
		...
		{
			//受信データを格納
			int nowSize = _recvDataMap.at(socket).size();
			_recvDataMap.at(socket).resize(nowSize + dataSize);
			std::memcpy((char *)&_recvDataMap.at(socket)[nowSize], &buf[0], dataSize);
			while (_recvDataMap.at(socket).size() > sizeof(int))
			{
				int dataSize = 0;
				std::memcpy(&dataSize, &_recvDataMap.at(socket)[0], sizeof(int));

				//受信データが一塊分あればキューリストに追加
				if (_recvDataMap.at(socket).size() > dataSize)
				{
					std::pair<int, std::vector<char>> addData;
					addData.first = socket;
					addData.second.resize(dataSize);
					std::memcpy(&addData.second[0], &_recvDataMap.at(socket)[sizeof(int)], dataSize);
					_recvDataQueList.push(addData);

					_recvDataMap.at(socket).erase(_recvDataMap.at(socket).begin(), _recvDataMap.at(socket).begin() + dataSize + sizeof(int));
				}
			}
		}
		else if (dataSize == 0)
...
//Class UDP_Routine
//===============================================================
void UDP_Routine::Update(std::shared_ptr<BaseSocket> _socket, std::queue<std::pair<sockaddr_in, std::vector<char>>> &_recvDataQueList)
{

	std::pair<sockaddr_in, std::vector<char>> addData;
	char buf[TCP_BUFFERSIZE];
